...
<body onload="onload_fun();">
    ...
    function delete_onclick(obj) {
        ...
        $.ajax({
            ...
            url: '../../DBOperation.php',
            data: {
                id: parseInt(trObj.children[0].innerHTML),
                log11: 1
            },
    ...
    function save_onclick(obj) {
        var trObj = obj.parentNode.parentNode.parentNode.parentNode;
        var rate = trObj.children[4].children[0];
        ...
        if (rate.value != "" && rate.value != "0") {
            if (numx.test(rate.value) == false) {
                ...
                rate.focus();
            } else if (numx.test(quantity.value) == false && quantity.value != "") {
                ...
                $.ajax({
                    ...
                    success: function(html) {
                        if (html == 0) {
                            ...
                            trObj.children[4].innerHTML = rate.value;
                            rate.remove();
                            trObj.children[3].innerHTML = quantity.value;
                            quantity.remove();
                        }
                    }
                });
    ...
    function edit_onclick(obj) {
        var trObj = obj.parentNode.parentNode.parentNode.parentNode;
        var rate = trObj.children[4].innerHTML;
        var quantity = trObj.children[3].innerHTML;
        var numx = /^[0-9.]+$/;
        if (numx.test(rate)) {
            ...
            trObj.children[3].innerHTML = "";
            var text = document.createElement("INPUT");
            text.type = "text";
            text.value = quantity;
            text.className = "form-control";
            text.setAttribute("name", "quantity");
            text.setAttribute("id", "quantity");
            text.setAttribute("placeholder", "TAKA No");
            text.setAttribute("style", "width:60px");
            trObj.children[3].appendChild(text);
        }
    }
    </script>
    <script>
